...
ReservationsCatalog reservations_catalog_create() {
  ...
  reservations_catalog->reservations_hash_table =
      g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, free_reservation);
...
void insert_reservation(ReservationsCatalog reservations_catalog, Reservation reservation) {
  g_hash_table_insert(reservations_catalog->reservations_hash_table, GINT_TO_POINTER(reservation_get_id(reservation)),
                      reservation);
}

Reservation get_reservation_by_id(ReservationsCatalog reservations_catalog, int reservation_id) {
  return (Reservation)g_hash_table_lookup(reservations_catalog->reservations_hash_table,
